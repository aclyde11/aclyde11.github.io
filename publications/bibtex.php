<?php
	...
	var $resultedHtml;
				...
				if (in_array("all", $projects) || $this->checkProject($i)) {
					switch ($this->types[$i]) {
						...
						case "techreport":
							...
							break;
						case "preprint":
							$this->htmlPublication("preprint", $preprint, $i);
							break;
						default:
		...
		function printPublications() {
            ...
            for($i = 0; $i < count($this->typeData); $i++) {
               if (isset($this->typeData[$i]) && $this->typeData[$i] == "preprints") {
                 echo '<li><a href="#" class="btn btn-xs btn-info" data-filter=".'.$this->typeData[$i].'">Preprints</a></li>';
               }
            }
